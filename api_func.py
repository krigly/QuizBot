...
def generate_quiz(text, client, count=5):
    prompt = rf"""
    Твоя задача — создать РОВНО {count} вопросов c вариантами ответов на основе текста.

    ТРЕБОВАНИЯ:
    1. Должно быть РОВНО {count} вопросов. Не больше и не меньше.
    2. Каждый вопрос должен иметь 4 варианта ответа (A, B, C, D).
    3. Ответ должен быть ТОЛЬКО JSON массивом из {count} объектов.
